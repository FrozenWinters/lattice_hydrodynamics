...
namespace distributed{
  ...
  namespace detail{
    ...
    struct _Comm_Tasks{
      ...
      template<size_t axis, int dir>
      static void _task_import(const algebra::xfield<T, buff_len, XS...>& A, Communicator<N>& comm);
    };
    ...
    template<size_t N, typename T, size_t buff_len, size_t... XS>
    void _Comm_Tasks<N, T, buff_len, XS...>::_task_import(const algebra::xfield<T, buff_len, XS...>& A, Communicator<N>& comm){
      constexpr size_t len = algebra::xfield<T, buff_len, XS...>::template bufferSize<axis>();
      T buff[len];
