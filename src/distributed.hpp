...
namespace distributed{
  ...
  namespace detail{
    ...
    public:
      ...
      std::enable_if_t<N != 1> sendToAdjacent(void* buff, const size_t& size, const size_t& axis, const int& dir);
      std::enable_if_t<N != 1> recvFromAdjacent(void* buff, const size_t& size, const size_t& axis, const int& dir);
  ...
  namespace detail{
    ...
    template<size_t N>
    std::enable_if_t<N != 1> Communicator<N>::sendToAdjacent(void* buff, const size_t& size, const size_t& axis, const int& dir){
      MPI_Send(buff, size, MPI_BYTE, this->nbrs[axis][dir == 1], MPI_ANY_TAG, MPI_COMM_WORLD);
    }

    template<size_t N>
    std::enable_if_t<N != 1> Communicator<N>::recvFromAdjacent(void* buff, const size_t& size, const size_t& axis, const int& dir){
